...
## データ構造
  ...
  - **`data`**: データ系列の配列です。
      - **`name`**: データ系列の名前（必須）。
      - **`value`**: 観測値や属性データの配列（オプション）。配列内の要素はすべて同じ型である必要があります。省略した場合、時空間IDの位置情報のみを記録します。
      - **`ids`**: 時空間IDの配列（必須）。`ref` インデックスを介して `value` 配列内のデータとマッピングされます。`value` がない場合は `ref` を指定する必要はありません。
...
コアとなる空間識別のためのオブジェクトは、以下のパラメータを使用します：
  ...
  - `i`, `t`: 時間に依存するデータのための時間インデックス。`t` は単一の値 `[t]` または範囲 `[start, end]` をサポートします。
  - `ref`: `value` 配列内のどのデータを指すかを示す参照インデックス（オプション。`value` がない場合は指定不要）。
